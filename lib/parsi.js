...
LR.prototype.closure = function(items) {
	var items = new Set(items.elements); // Fuck mutable sets!
		...
		for(var i = 0; i < state.items.length; i++) {
			...
			if(item.markedSymbol !== undefined) {
				...
				if(items.contains(newState.items)) { // @todo Much ugly!
					for(var n = 0; n < items.elements.length; n++) {
						if(JSON.stringify(items.elements[n]) == JSON.stringify(newState.items)) {
							state.addMove(n, item.markedSymbol);
							break;
						}
					}
				}
...
function Item(production, marker, lookahead) {
	...
	this.marker = marker;
	this.lookahead = lookahead; // array
